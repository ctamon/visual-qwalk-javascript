...
// adapted from http://www.akiti.ca/compRealEigSys.js
...
function hqr2(N, A, B, low, igh, wi, wr, oPar) {
  ...
  var k = 0, l, m, mp2
  var en = igh, incrFlag = 1, its, itn = 30 * N, enm2, na, notlas
  ...
  while (en >= low) {
    ...
    for (m = enm2; m >= l; m--) {
      ...
      tst1 = Math.abs(A[m-1][m-1]) + Math.abs(zz) + Math.abs(A[m+1][m+1]);
      tst1 *= Math.abs(p);
